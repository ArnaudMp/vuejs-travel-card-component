...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  ...
  _push(ssrRenderComponent(_component_NuxtLink, mergeProps({
    ...
    default: withCtx((_, _push2, _parent2, _scopeId) => {
      if (_push2) {
        _push2(`<div class="travel-card overflow-hidden rounded-3xl bg-cover" style="${ssrRenderStyle({ backgroundImage: `url(img/${$setup.post.id}.jpg)` })}"${_scopeId}><div class="bg-white/50 p-5 text-left text-black transition-colors duration-500 ease-in-out hover:bg-black/30 hover:text-white sm:p-20"${_scopeId}><div class="grid grid-cols-2 justify-between"${_scopeId}><div class="title" data-swiper-parallax="-300"${_scopeId}><p class="text-4xl font-bold"${_scopeId}>${ssrInterpolate($setup.post.title.slice(0, 25))}</p></div><div class="flex flex-wrap justify-end"${_scopeId}><div class="tag px-4 py-2 m-1 rounded-3xl border-2 border-orange-500 bg-orange-500 text-white"${_scopeId}><span${_scopeId}>Popular destination</span></div><div class="tag px-4 py-2 m-1 rounded-3xl border-2 border-green-500 text-white bg-green-500"${_scopeId}><span${_scopeId}>Best for adventure</span></div></div></div><div class="subtitle pt-10" data-swiper-parallax="-200"${_scopeId}><p class="text-2xl font-light"${_scopeId}>${ssrInterpolate($setup.post.body.slice(0, 40))}</p></div><div class="text pt-10" data-swiper-parallax="-100"${_scopeId}><p class="text-lg"${_scopeId}>${ssrInterpolate($setup.post.body)} ${ssrInterpolate($setup.post.body)}</p></div><div class="mt-auto flex pl-1 pt-10"${_scopeId}><div class="button border-3 mb-2 me-2 rounded-lg border border-black px-5 py-2.5 text-center text-sm font-medium text-black"${_scopeId}> Voir les activit\xE9s </div></div></div></div>`);
      } else {
        return [
          createVNode("div", {
            ...
            createVNode("div", { class: "bg-white/50 p-5 text-left text-black transition-colors duration-500 ease-in-out hover:bg-black/30 hover:text-white sm:p-20" }, [
              createVNode("div", { class: "grid grid-cols-2 justify-between" }, [
                createVNode("div", {
                  ...
                  createVNode("p", { class: "text-4xl font-bold" }, toDisplayString($setup.post.title.slice(0, 25)), 1)
                ]),
                createVNode("div", { class: "flex flex-wrap justify-end" }, [
                  createVNode("div", { class: "tag px-4 py-2 m-1 rounded-3xl border-2 border-orange-500 bg-orange-500 text-white" }, [
                    createVNode("span", null, "Popular destination")
                  ]),
                  createVNode("div", { class: "tag px-4 py-2 m-1 rounded-3xl border-2 border-green-500 text-white bg-green-500" }, [
              ...
              createVNode("div", {
                ...
                createVNode("p", { class: "text-2xl font-light" }, toDisplayString($setup.post.body.slice(0, 40)), 1)
              ]),
              createVNode("div", {
                class: "text pt-10",
                "data-swiper-parallax": "-100"
              }, [
                createVNode("p", { class: "text-lg" }, toDisplayString($setup.post.body) + " " + toDisplayString($setup.post.body), 1)
              ]),
...
//# sourceMappingURL=TravelCard-dUsva49Y.mjs.map
